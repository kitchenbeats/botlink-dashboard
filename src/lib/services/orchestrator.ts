import { executeAgentSDK } from './agent-executor';
import { getSystemAgentById } from './system-agents';
import { executionEvents } from './event-emitter';
import { saveExtractedProject } from './code-extractor';
import {
  createAgent,
  createTask,
  createTasks,
...
} from '@/lib/db/structured-outputs';
import {
...
export class WorkflowOrchestrator {
  ...
  async run(): Promise<void> {
    try {
      ...
      this.emitEvent('agent_start', { agent: 'task-planner', message: 'Planning tasks...' });
      const planOutput = await executeAgentSDK(plannerAgent, planningTask);
      this.emitEvent('agent_complete', { agent: 'task-planner', message: 'Task plan created' });

      // Step 4: Parse and save the plan as structured data
      const parsedPlan = this.parsePlan(planOutput);

      // Save task plan to structured outputs
      await saveTaskPlan({
        task_id: planningTask.id,
        overall_strategy: parsedPlan.overall_strategy,
        tasks: parsedPlan.tasks,
      });

      // Save planned tasks as individual structured outputs
      await savePlannedTasks(
        parsedPlan.tasks.map((task, index) => ({
          task_id: planningTask.id,
          sequence: index,
          title: task.title,
      ...
      this.emitEvent('agent_start', { agent: 'orchestrator', message: 'Creating specialized agents...' });
      const orchestratorOutput = await executeAgentSDK(orchestratorAgent, orchestratorTask);
      ...
      await updateTask(orchestratorTask.id, {
        status: 'completed',
        output: JSON.stringify({ agent_ids: specializedAgents.map((a) => a.id) }),
        completed_at: new Date().toISOString(),
      });

      // Step 10: Create workflow with nodes and edges
      this.emitEvent('execution_update', { message: 'Creating workflow visualization...' });
      const workflow = await this.createWorkflowFromPlan(parsedPlan, specializedAgents);
  ...
  async resume(): Promise<void> {
    try {
      ...
      if (!executionData.plan || !executionData.specialized_agents) {
        throw new Error('Invalid execution data: missing plan or specialized_agents');
      }
      ...
      await this.executeTasksWithDependencies(tasks, specializedAgents, plan);
      ...
      await updateExecution(this.execution.id, {
        ...
        completed_at: new Date().toISOString(),
        output: JSON.stringify({
          tasks: await getTasks(this.execution.id),
        }),
      });
    } catch (error) {
      await updateExecution(this.execution.id, {
  ...
  private async executeTasksWithDependencies(
    ...
    const executeTask = async (taskIndex: number): Promise<void> => {
      ...
      if (!task || !agent) throw new Error(`Task or agent not found at index ${taskIndex}`);
      ...
      try {
        const projectId = await saveExtractedProject(
          this.teamId,
          this.execution.id,
          taskOutput,
          this.execution.input
        ...
        if (projectId) {
          this.emitEvent('execution_update', {
            message: `Code extracted and saved to project ${projectId}`,
            projectId,
          });
        }
      } catch (error) {
        console.error('Failed to extract code:', error);
        // Don't fail the task if code extraction fails
      }
  ...
  private async executeTaskWithLogicCheck(agent: Agent, task: Task): Promise<string> {
    const maxAttempts = 3;
    let lastOutput = '';
  ...
  private parseOrchestratorOutput(output: string): InsertAgent[] {
    ...
    return parsed.agents.map((agent) => ({
      ...
      config: agent.config || {}, // Ensure config is not undefined
    }));
  }

  private async createSpecializedAgents(agentSpecs: InsertAgent[]): Promise<Agent[]> {
    // Create all agents in parallel for better performance
    return Promise.all(agentSpecs.map((spec) => createAgent(spec)));
  }
  ...
  private emitEvent(type: 'execution_update' | 'task_update' | 'agent_start' | 'agent_complete' | 'agent_error', payload: unknown) {
  ...
  private calculateTaskLevels(tasks: TaskPlannerOutput['tasks']): number[] {
    ...
    const calculateLevel = (index: number, visited = new Set<number>()): number => {
      ...
      const maxDepLevel = Math.max(
        ...dependencies.map(depIndex => calculateLevel(depIndex, new Set(visited)))
      );
  ...
  private async createWorkflowFromPlan(plan: TaskPlannerOutput, agents: Agent[]) {
