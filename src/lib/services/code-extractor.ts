import { createProject, createFiles } from '@/lib/db';
 ...
 */
export function inferFilename(codeBlock: ExtractedCodeBlock, index: number): string {
  ...
  if (codeBlock.code.includes('function App(') || codeBlock.code.includes('export default function')) {
    return `App${ext}`;
  }
  if (codeBlock.code.includes('function main(') || codeBlock.code.includes('def main(')) {
    return `main${ext}`;
  }
 ...
 */
export function detectProjectType(blocks: ExtractedCodeBlock[]): ProjectType {
  const languages = blocks.map(b => b.language.toLowerCase());

  if (languages.some(l => l.includes('html') || l.includes('css') || l.includes('tsx') || l.includes('jsx'))) {
    return 'web';
  }
  if (languages.some(l => l.includes('python') && blocks.some(b => b.code.includes('fastapi') || b.code.includes('flask')))) {
    return 'api';
  }
  if (languages.some(l => l.includes('rust') || l.includes('go')) && blocks.some(b => b.code.includes('fn main'))) {
    return 'cli';
  }
 ...
 */
export function extractProject(text: string, executionInput: string): ExtractedProject | null {
  ...
  return {
    ...
    type: detectProjectType(codeBlocks),
    files,
  };
}
...
export async function saveExtractedProject(
  ...
  const project = await createProject({
    ...
    template: extractedProject.type as import('@/lib/types/database').ProjectTemplate,
